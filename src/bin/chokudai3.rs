                ...
                if t == 0 || (flip_x == bestflip_x && flip_y == bestflip_y) {
                    let out = if *SWAP > 0 {
                        let out = chokudai1::solve_swap2(
                            ...
                            8,
                            &init_canvas,
                            &|png, init_canvas| chokudai_dev2::monte_solve2(png, sec3, init_canvas),
                        )
                        .1;
                        (
                            init_canvas
                        ...
                        let out = chokudai_dev2::monte_solve2(&png, sec3, &init_canvas).1;
                        (
                            init_canvas
                                .clone()
                                .apply_all_and_score(out.clone(), &png)
                    ...
                    if best.0.setmin(out.0) {
                        ...
                        bestflip_y = flip_y;
                        best.1 = out.1;
                        eprintln!("{} {} {}", best.0, bestflip_x, bestflip_y);
                    }
                }
