use icfpc2022::{read_png, wata::solve};
...
fn main() {
    ...
    for k in 0..4 {
        ...
        tmp_v.sort();
        target_color[k] = tmp_v[tmp_v.len() / 2];
    }
    ...
    for y in 0..campus_size {
        for x in 0..campus_size {
            sum_x[x] += get_diff(target_color, png[y][x]);
        }
    }
    ...
    loop {
        ...
        if bestpos != 9999 {
            ...
            swap_x(bestpos - diff, last - diff, diff, &mut png, &mut sum_x);
            last -= bestfind;
        }
    }
    ...
    for y in 0..campus_size {
        for x in 0..campus_size {
            sum_y[y] += get_diff(target_color, png[y][x]);
        }
    }
    ...
    loop {
        ...
        if bestpos != 9999 {
            ...
            swap_y(bestpos - diff, last - diff, diff, &mut png, &mut sum_y);
            last -= bestfind;
        }
    }

    let (out, moves) = solve(&png);
    eprintln!("{}", out);
